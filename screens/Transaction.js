...
export default class TransactionScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    var transactionType = await this.checkBookAvaliability(bookId);

    if (!transactionType){
      this.setState({bookId: "", studentId: ""});
      //Apenas para usuarios do android
      ...
      //Apenas para usuarios do android
      //ToastAndroid.show("O livro foi entregue ao aluno!", ToastAndroid.SHORT);
      ...
      var isElegible = await this.checkStudentEligibilityForBookReturn(bookId);
      if (isElegible) {
        var {bookName, studentName} = this.state;
  ...
  checkBookAvaliability = async bookId => {
    ...
    if (bookRef.docs.length == 0) {
      ...
      bookRef.docs.map(doc => {
        // Se o livro estiver disponível, o tipo de transção será entrega. Caso contrário, será retorno.
    ...
    if (studentRef.docs.length == 0){
      ...
      studentRef.docs.map(doc => {
        if (doc.data().number_of_books_issued < 2){
          ...
          isStudentElegible = false;
          Alert.alert("O aluno já retirou o maximo de livros permitido pela biblioteca!");
          this.setState({
            bookId: "",
            studentId: ""
          });
